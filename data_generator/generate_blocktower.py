...
class Generator:
    ...
    def save(self, ab, cd, colors, do_op, cf_cubes, n):
        ...
        # Tower类内没有confounders，混杂实际上是masses
        # assert ab.confounders == cd.confounders
        assert ab.masses == cd.masses
...
class Simulator:
    ...
    def _init(self, tower, colors, plane_id):
        ...
        # Load ground
        self.p.loadURDF(f"../data_generator/urdf/plane_" + str(plane_id) + "/plane.urdf",
                        useMaximalCoordinates=True)
        list_cube = []
        # Add cubes
        for i in range(tower.n_cubes):
            color = colors[i]
            cube = self.p.loadURDF(f"../data_generator/urdf/{color}/cube.urdf",
