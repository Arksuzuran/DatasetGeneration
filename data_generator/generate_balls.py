...
def check_counterfactual(alt_cd, cd, mass_permutation):
    ...
    # for each ball, try to change its confounders
...
class Arena:
    ...
    def generate_random_do_operation(self):
        """Return a do-operation candidate: {"operation", "cube", "amplitude"}
            do-interventions consist in
            (a) removing the top cube or a ball, or
            (b) shifting a cube/ball on the horizontal plane
        """
        ...
        # Apply do-op
        # shift cube ball_idx in the given direction
...
class Simulator:
    ...
    def run(self, pipe, arena, plane_id, colors, rendering=False):
        ...
        # Logs
        # seq_states: time_step x ball x (位置坐标, 运动方向, 速度, 角速度)
        ...
        # 状态信息，rgb图像，深度图（像素点距离摄像机的距离），分割图（标记模拟场景中不同物体或区域）
    ...
    def _init(self, arena, colors, plane_id):
        ...
        # Load ground
        pb.loadURDF(f"../data_generator/urdf/plane_{plane_id}/plane.urdf", useMaximalCoordinates=True)

        # Walls
        limit = 4.3
        angle = (np.pi / 2.)
        pb.loadURDF(f"../data_generator/urdf/plane_white/plane.urdf", [0, limit, 0],
                    pb.getQuaternionFromEuler([angle, 0, 0]), useMaximalCoordinates=True)
        pb.loadURDF(f"../data_generator/urdf/plane_white/plane.urdf", [0, -limit, 0],
                    pb.getQuaternionFromEuler([-angle, 0, 0]), useMaximalCoordinates=True)
        pb.loadURDF(f"../data_generator/urdf/plane_white/plane.urdf", [limit, 0, 0],
        ...
        for i in range(arena.n_balls):
            ...
            cube = self.p.loadURDF(f"../data_generator/urdf/{color}/ball.urdf",
                                   [x, y, 0.5],
